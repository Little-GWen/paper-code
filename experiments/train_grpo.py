...
@ray.remote
class GRPOWorker:
    def __init__(self, env_id, seed, worker_id, adv_mode="tiered"):
        ...
        # Worker 只负责推理，device='cpu' 通常更快且省显存
...
def train():
    ...
    ray.init()
    save_dir = f'results/train/grpo_main/seed_{args.seed}'
    ...
    learner = Agent_GRPO_Ray(
        ...
        BATCH_SIZE, LEARNING_RATE, DECAY_MAX_STEP, GAMMA, EPS_CLIP, K_EPOCHS,
        ENTROPY_COEF, DEVICE, is_worker=False, adv_mode="tiered", use_dynamic_beta=True
    )

    workers = [GRPOWorker.remote(RAM_ENV_NAME, args.seed, i, "tiered") for i in range(NUM_PROCESSES)]

    global_base, ema = 0.0, 0.95
    step, ep = 0, 0

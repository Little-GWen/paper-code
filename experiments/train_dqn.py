...
class MatchCallback(BaseCallback):
    ...
    def _save_logs(self):
        if len(self.log_rew) > 0:
            print(f"   [DQN] Steps: {self.num_timesteps}/{TOTAL_TRAIN_STEPS} | Mean Rew: {np.mean(self.log_rew[-100:]):.2f}")
...
def main():
    ...
    print(f"🚀 DQN (Ray) Training Started | Workers: {args.n_workers} | Target Steps: {TOTAL_TRAIN_STEPS}")
    try:
        # 使用统一的总步数
